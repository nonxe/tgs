    ...
    <section className={`flex-1 flex flex-col w-full gap-6 ${embed ? "py-2" : "px-4 py-8 max-w-4xl mx-auto"}`}>
      ...
      {!embed && (
        <div className="text-center md:text-left">
          <h2 className="text-[30px] md:text-[38px] font-black tracking-tight leading-tight select-none">
            Claude 4.5 Haiku AI.
            <br />
            <span className="opacity-40">Fast, intelligent assistant chat.</span>
          </h2>
          <p className="mt-1.5 text-sm text-muted-foreground max-w-md">
            Powered by Anthropic Claude 4.5 Haiku for code generation, writing, math, and general reasoning.
          </p>
        </div>
      )}
...
function ChatTool() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`https://apis.davidcyril.name.ng/ai/claude-haiku-4.5?prompt=${encodeURIComponent(userMessage)}`);
      ...
      setMessages((prev) => [
        ...prev, 
        { role: "assistant", content: "Error: Unable to connect to Claude 4.5 Haiku API. Please try again shortly." }
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="rounded-2xl border border-border/80 p-5 bg-card flex flex-col h-[560px] relative overflow-hidden shadow-2xl">
      {/* Active Model Header */}
      <div className="flex items-center justify-between pb-3.5 border-b border-border/50 select-none flex-shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-foreground">Claude 4.5 Haiku</span>
              <span className="px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[9.5px] font-mono font-bold">
                Anthropic
              </span>
            </div>
            <p className="text-[10px] text-muted-foreground font-mono">Fast & Intelligent Assistant</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-4 pr-1 scrollbar-thin">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center text-muted-foreground p-6 select-none">
            <div className="size-12 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400 mb-3">
              <Bot className="size-6" />
            </div>
            <p className="text-sm font-bold text-foreground">Claude 4.5 Haiku AI Assistant</p>
            <p className="text-xs text-muted-foreground max-w-sm mt-1 leading-relaxed">
              Ask anything about coding, creative writing, analysis, math, or reasoning.
            </p>
          </div>
        ) : (
          messages.map((msg, idx) => (
            <div key={idx} className={`flex items-start gap-3 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
              <div className={`size-7 rounded-xl border flex items-center justify-center flex-shrink-0 ${
                msg.role === "user" ? "bg-foreground border-foreground text-background" : "bg-amber-500/10 border-amber-500/30 text-amber-400"
              }`}>
        ...
        {loading && (
          <div className="flex items-start gap-3">
            <div className="size-7 rounded-xl border bg-amber-500/10 border-amber-500/30 text-amber-400 flex items-center justify-center">
            ...
            <div className="bg-secondary/40 border border-border/50 rounded-2xl px-4 py-3 flex items-center gap-1.5">
              ...
              <span className="size-1.5 bg-amber-400 rounded-full animate-bounce [animation-delay:0.2s]" />
              <span className="size-1.5 bg-amber-400 rounded-full animate-bounce [animation-delay:0.4s]" />
            </div>
          </div>
        )}
        <div ref={scrollRef} />
      ...
      <form onSubmit={handleSend} className="pt-3 border-t border-border/50 flex gap-2 flex-shrink-0 select-none">
        <input
          type="text"
          placeholder="Ask Claude Haiku 4.5 anything..."
          value={input}
